...
enum class UnaryOp { NOT, MINUS, PLUS, IS_NULL, IS_NOT_NULL };
...
struct Expression {
  ...
  Expression()
      : type(ExprType::LITERAL_NULL), binary_op(BinaryOp::EQ),
        unary_op(UnaryOp::NOT) {}

  static std::shared_ptr<Expression> makeColumnRef(const string &col,
                                                   const string &tbl = "") {
  ...
  makeBinaryOp(BinaryOp op, std::shared_ptr<Expression> left,
  ...
  static std::shared_ptr<Expression>
  makeUnaryOp(UnaryOp op, std::shared_ptr<Expression> operand) {
    auto e = std::make_shared<Expression>();
  ...
  static std::shared_ptr<Expression>
  makeFunctionCall(const string &name,
...
struct RANode {
  ...
  std::vector<std::vector<ExprPtr>> insert_values;
  ...
  RANode()
      : type(RANodeType::TABLE_SCAN), select_all(false), limit_count(-1),
        offset_count(0), is_distinct(false) {}
  RANode(RANodeType t)
      : type(t), select_all(false), limit_count(-1), offset_count(0),
        is_distinct(false) {}

  static RANodePtr makeTableScan(const string &table,
                                 const string &alias = "") {
  ...
  static RANodePtr makeJoin(RANodeType join_type, RANodePtr left,
                            RANodePtr right, ExprPtr cond = nullptr) {
  ...
  static RANodePtr makeGroupBy(RANodePtr child, std::vector<ExprPtr> groups,
  ...
  static RANodePtr makeLimit(RANodePtr child, int64_t limit,
                             int64_t offset = 0) {
...
inline void tree_print_helper(SqlNode *root, const string &prefix = "",
  ...
  std::cout << root->type;
  if (root->alias != "")
    std::cout << " { alias: " << root->alias << " }";
  if (root->qualifier != "")
    std::cout << " { qualifier: " << root->qualifier << " }";
  std::cout << std::endl;
...
inline string ra_type_to_string(RANodeType type) {
  ...
  case RANodeType::FULL_JOIN:
    return "FullJoin";
  case RANodeType::UNION_OP:
    return "Union";
  case RANodeType::INTERSECT_OP:
    return "Intersect";
  case RANodeType::EXCEPT_OP:
    return "Except";
...
inline string expr_to_string(const ExprPtr &expr) {
  ...
  case ExprType::UNARY_OP: {
    ...
    case UnaryOp::IS_NULL:
      return expr_to_string(expr->children[0]) + " IS NULL";
    case UnaryOp::IS_NOT_NULL:
      return expr_to_string(expr->children[0]) + " IS NOT NULL";
    }
...
inline void ra_tree_print_helper(const RANodePtr &node,
  ...
  case RANodeType::PROJECT:
    ...
    if (node->select_all) {
      ...
      for (size_t i = 0; i < node->projections.size(); i++) {
        ...
        if (i < node->projection_aliases.size() &&
            !node->projection_aliases[i].empty())
