// I initially intended to get this done, but it doesn't really work
...
TLogger::TLogger(PageCache &cache, Database &database, TScheduler &scheduler)
    : theCache(cache), theDatabase(database), theScheduler(scheduler),
...
void TLogger::clear_ops() { theOps.clear(); }
